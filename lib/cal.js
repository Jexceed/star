    ...
    return request(conf.cal, function(e, r, body) {
        ...
        $tr.each(function(){
            ...
            if($td.length !== 0){
                ...
                concept = "";
                if($td.eq(2).text().replace(/&nbsp;+|\s+/g, '')){
                    concept= "[相关概念:"+$td.eq(2).text().replace(/&nbsp;+/g, ',').replace(/\s+/g, '')+"]";
                }
                console.log('  ', date, '  ', dayOfW, '   ', _.trimLeft(_.trim(evt)), '   ', _.trimLeft(_.trim(concept.grey)));
            }
        });
